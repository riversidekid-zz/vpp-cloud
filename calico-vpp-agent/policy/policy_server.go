...
func (s *Server) handleHostMetadataV4V6Update(msg *proto.HostMetadataV4V6Update, pending bool) (err error) {
	...
	ip4net.IP = ip
	ip, ip6net, err := net.ParseCIDR(msg.Ipv6Addr + "/32")
	if err != nil {
		return err
	}
	ip6net.IP = ip
...
func (s *Server) handleWireguardEndpointUpdate(msg *proto.WireguardEndpointUpdate, pending bool) (err error) {
	...
	_, ok := s.nodeStatesByName[msg.Hostname]
	if ok { //node already received
		old := &common.LocalNodeSpec{
			ASNumber:           s.nodeStatesByName[msg.Hostname].ASNumber,
			IPv4Address:        s.nodeStatesByName[msg.Hostname].IPv4Address,
			IPv6Address:        s.nodeStatesByName[msg.Hostname].IPv6Address,
			Labels:             s.nodeStatesByName[msg.Hostname].Labels,
			Name:               s.nodeStatesByName[msg.Hostname].Name,
			WireguardPublicKey: s.nodeStatesByName[msg.Hostname].WireguardPublicKey}
		s.nodeStatesByName[msg.Hostname].WireguardPublicKey = msg.PublicKey
		err = s.onNodeUpdated(old, s.nodeStatesByName[msg.Hostname])
		if err != nil {
			return err
		}
		s.nodeStatesByName[msg.Hostname].WireguardPublicKey = msg.PublicKey
	} else {
		s.nodeStatesByName[msg.Hostname] = &common.LocalNodeSpec{Name: msg.Hostname, WireguardPublicKey: msg.PublicKey}
	}
	return nil
}
